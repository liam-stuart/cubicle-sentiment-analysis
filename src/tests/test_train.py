...
def test_train_model_runs():
    _, epochs = train_model(model, "GRU", "cpu", 0.01, 3, 4, train_loader, val_loader, callback)
    assert epochs == 3
...
def test_train_model_stops_early(mock_early_stopper):
    ...
    results, epochs = train_model(model, "GRU", "cpu", 0.01, 3, 4, train_loader, val_loader, callback)
